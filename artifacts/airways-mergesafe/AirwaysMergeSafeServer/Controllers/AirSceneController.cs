...
///   P6  — Index passes RecentEvents + ground/air counts to AirSceneViewModel.
...
public class AirSceneController : Controller
{
        ...
        bool simOnly = false)
    {
        ...
        if (zones.Count == 0)
        {
            TraceLogger.Info("AirScene", nameof(GetAnimationData), $"No zones for hw={highwayId}");
        ...
        TraceLogger.Info("AirScene", nameof(GetAnimationData), $"Highway={highwayId} zones={zones.Count} isEW={isEW}");
